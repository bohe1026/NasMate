...
export function KnowledgeIndexPanel() {
  ...
  async function rebuild() {
    ...
    setRebuildMessage('')
    try {
      const data = await apiFetch('/api/index/rebuild', { method: 'POST' }).then(responseJSON<{ summary?: string; itemCount?: number; readOnly?: boolean }>)
      setRebuildMessage(`${data.summary || '元数据索引已重建'} · ${data.itemCount ?? 0} 项`)
