...
import { AlertTriangle, Activity, Archive, CheckCircle2, Container, ShieldCheck } from 'lucide-react'
...
export function HealthReportPanel() {
  ...
  useEffect(() => {
    void readReport().then((data) => { setReport(data); setState('ready') }).catch(() => setState('error'))
  }, [])
  ...
  return <section className="readonly-panel health-snapshot" aria-labelledby="health-snapshot-title">
    <header className="readonly-panel-heading"><div><span className="readonly-icon mint"><Activity size={18} /></span><div><h2 id="health-snapshot-title">健康日报快照</h2><p>汇总当前授权范围内的存储、容器、备份和任务状态。</p></div></div><span className="status-tag">只读报告</span></header>
