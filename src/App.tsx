...
import {
  ...
  MessageSquare, Monitor, Palette, PanelLeft, Plus, RotateCcw, Search, ShieldCheck,
  Sparkles, SquareTerminal, X,
} from 'lucide-react'
import { apiFetch } from './api'
import { formatBytes } from './format'
import { SourceProbePanel } from './SourceProbePanel'
import { OrganizePanel } from './OrganizePanel'
import { DownloadPlanner } from './DownloadPlanner'
...
type Task = { id: string; prompt: string; status: string; summary: string; updatedAt: string; parentTaskId?: string }
...
type RecoveryPlan = { status: string; summary: string; backupName: string; target: string; sampledFiles: number; steps: string[]; willOverwrite: boolean; requiresApproval: boolean; readOnly: boolean }
...
function App() {
  ...
  const taskPageRequest = useRef(0)
  ...
  useEffect(() => {
    ...
    void apiFetch('/api/downloads?limit=30&offset=0').then(readJSON<{ items: DownloadPlan[]; truncated?: boolean; nextOffset?: number }>).then((data) => { if (live) { setPlans(data.items ?? []); setPlansHasMore(Boolean(data.truncated)); setPlansOffset(data.nextOffset ?? (data.items?.length ?? 0)); setPlansState('ready') } }).catch(() => { if (live) setPlansState('error') })
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      void apiFetch('/api/downloads?limit=30&offset=0').then(readJSON<{ items: DownloadPlan[]; truncated?: boolean; nextOffset?: number }>).then((data) => { setPlans(data.items ?? []); setPlansHasMore(Boolean(data.truncated)); setPlansOffset(data.nextOffset ?? (data.items?.length ?? 0)) }).catch(() => undefined)
    }, 5000)
    return () => window.clearInterval(timer)
  }, [page, plans])

  async function loadMorePlans() {
    if (plansLoadingMore || !plansHasMore) return
    setPlansLoadingMore(true)
    const requestedOffset = plansOffset
    try {
      const data = await apiFetch(`/api/downloads?limit=30&offset=${requestedOffset}`).then(readJSON<{ items: DownloadPlan[]; truncated?: boolean; nextOffset?: number }>)
      const next = data.items ?? []
      setPlans((current) => [...current, ...next.filter((item) => !current.some((existing) => existing.id === item.id))])
      setPlansHasMore(Boolean(data.truncated))
      setPlansOffset(data.nextOffset ?? requestedOffset + next.length)
    } finally { setPlansLoadingMore(false) }
  }
  useEffect(() => {
  ...
  async function loadMoreTasks() {
    ...
    const requestId = ++taskPageRequest.current
    const requestedOffset = tasksOffset
    try {
      const data = await apiFetch(`/api/tasks?limit=100&offset=${requestedOffset}`).then(readJSON<{ items: Task[]; truncated?: boolean; nextOffset?: number }>)
      if (requestId !== taskPageRequest.current) return
      setTasks((current) => [...current, ...(data.items ?? []).filter((item) => !current.some((existing) => existing.id === item.id))]); setTasksHasMore(Boolean(data.truncated)); setTasksOffset(data.nextOffset ?? requestedOffset + (data.items?.length ?? 0))
    } catch { if (requestId === taskPageRequest.current) setError('更早的任务暂时无法读取。') }
    finally { if (requestId === taskPageRequest.current) setTasksLoadingMore(false) }
  }
  ...
  async function generateHealthReport() {
    ...
    try {
      await apiFetch('/api/reports/health/generate', { method: 'POST' }).then(readJSON<{ artifact: string }>)
      const data = await apiFetch('/api/artifacts').then(readJSON<{ items: Artifact[] }>)
      setArtifacts(data.items ?? [])
    } catch { setError('健康报告生成失败，请稍后重试。') } finally { setGeneratingReport(false) }
  }
  async function loadRecoveryPlan() {
    if (recoveryState === 'loading') return
    setRecoveryState('loading'); setError('')
    try {
      const plan = await apiFetch('/api/backups/recovery-plan').then(readJSON<RecoveryPlan>)
      setRecoveryPlan(plan); setRecoveryState('ready')
    } catch { setRecoveryState('error'); setError('恢复演练计划暂时无法读取，请稍后重试。') }
  }
  async function cancelTask(task: Task) {
    if (task.status === '已完成' || task.status === '失败' || task.status === '已取消') return
    try {
      const updated = await apiFetch(`/api/tasks/${encodeURIComponent(task.id)}/cancel`, { method: 'POST' }).then(readJSON<Task>)
      setTasks((current) => current.map((item) => item.id === updated.id ? updated : item))
      setEvents([]); setEventsState('loading'); setEventsRevision((value) => value + 1)
    } catch { setError('任务取消失败，请稍后重试。') }
  }
  async function resumeTask(task: Task) {
    setError('')
    try {
      const resumed = await apiFetch(`/api/tasks/${encodeURIComponent(task.id)}/resume`, { method: 'POST' }).then(readJSON<Task>)
      setTasks((current) => [resumed, ...current.filter((item) => item.id !== resumed.id)])
      selectTask(resumed.id)
    } catch { setError('任务继续失败，请稍后重试。') }
  }
  ...
  return (
    <div className={`workspace ${collapsed ? 'is-collapsed' : ''}`} style={{ '--accent': accent } as CSSProperties}>
      ...
      <aside ref={sidebar} className={`workspace-sidebar ${mobileOpen ? 'is-open' : ''}`} aria-label="工作区导航">
        ...
        <div className="history-section">
          ...
          <div className="history-list">
            ...
            {tasksHasMore && !historySearch && <button className="text-button history-more" onClick={() => void loadMoreTasks()} disabled={tasksLoadingMore}>{tasksLoadingMore ? '正在读取…' : '加载更早任务'}</button>}
      ...
      <main id="workspace-main" className="workspace-main">
        ...
        {page === 'skills' && <section className="utility-content"><div className="utility-heading"><span className="welcome-eyebrow">MADE FOR YOUR NAS</span><h1>日常小事，各有所长。</h1><p>选择一项能力，描述具体需求后再发送任务。</p></div><div className="capability-grid">{capabilities.map(({ title, description, prompt, icon: Icon, tone, tag }) => <button className={`capability-card ${tone}`} key={title} onClick={() => handleSuggestion(prompt)}><span className="capability-art"><Icon size={27} /></span><ArrowUpRight className="card-arrow" size={17} /><strong>{title}</strong><p>{description}</p><span className="card-tag">{tag}</span></button>)}</div><FileSearchPanel /><DockerPanel /><BackupPanel /><KnowledgeIndexPanel /><OrganizePanel /><div className="info-strip"><ShieldCheck size={18} /><p>容器和备份能力取决于 NAS 的实际接入状态。暂不可用时，任务会明确提示。</p></div></section>}
        {page === 'health' && <section className="utility-content"><div className="utility-heading"><span className="welcome-eyebrow">A LITTLE MORE ROOM</span><h1>空间，一目了然。</h1><p>授权范围内的存储使用情况。</p><div className="heading-actions"><button className="primary-button report-button" onClick={() => void generateHealthReport()} disabled={generatingReport}>{generatingReport ? '正在生成…' : '生成健康报告'}</button><button className="secondary-button" onClick={() => void loadRecoveryPlan()} disabled={recoveryState === 'loading'}>{recoveryState === 'loading' ? '正在读取…' : '查看恢复演练计划'}</button></div></div>{error && <p className="inline-error" role="alert">{error}</p>}{usageState !== 'ready' ? <div className="empty-panel"><HardDrive size={32} /><h2>{usageState === 'loading' ? '正在读取存储状态' : '暂时无法读取存储'}</h2><p>{usageState === 'loading' ? '大目录的首次统计可能需要一些时间。' : '请检查应用连接，并在 UGOS 应用设置中授权文件夹。'}</p></div> : <><div className="volume-grid">{usage?.volumes?.map((volume, index) => <div className="volume-card" key={`${volume.name}-${index}`}><span><HardDrive size={19} />{volume.name}</span><h2>{formatBytes(volume.usedBytes)}<small> / {formatBytes(volume.totalBytes)}</small></h2><div className="usage-track"><span style={{ width: `${Math.max(0, Math.min(100, volume.totalBytes ? volume.usedBytes / volume.totalBytes * 100 : 0))}%` }} /></div><p>剩余可用 {formatBytes(volume.freeBytes)}</p></div>)}</div><div className="directory-list"><h2>空间占用排行</h2>{usage?.topItems?.map((item, index) => <div key={`${item.path}-${index}`}><FolderOpen size={17} /><span>{item.path}</span><strong>{formatBytes(item.sizeBytes)}</strong></div>)}</div></>}{recoveryPlan && <article className="recovery-card"><header><h2><ShieldCheck size={19} />恢复演练计划</h2><span className="status-tag">只读 · 需确认</span></header><p>{recoveryPlan.summary}</p><dl><div><dt>备份</dt><dd>{recoveryPlan.backupName || '—'}</dd></div><div><dt>目标</dt><dd>{recoveryPlan.target || '—'}</dd></div><div><dt>抽样文件</dt><dd>{recoveryPlan.sampledFiles}</dd></div></dl><ol>{recoveryPlan.steps.map((step, index) => <li key={`${step}-${index}`}>{step}</li>)}</ol><small>当前仅生成计划，不会覆盖原文件或执行恢复。</small></article>}<div className="directory-list"><h2>健康报告</h2>{artifacts.length ? artifacts.map((artifact) => <div key={artifact.name}><History size={17} /><span>{artifact.name}</span><strong>{formatBytes(artifact.sizeBytes)}</strong></div>) : <div><History size={17} /><span>暂无已生成报告</span></div>}</div><HealthReportPanel /></section>}
        {page === 'downloads' && <section className="utility-content"><div className="utility-heading"><span className="welcome-eyebrow">EVERY FILE HAS A HOME</span><h1>下载，心中有数。</h1><p>查看任务状态，并确认每一次文件写入。</p></div><NetworkSearchPanel onSelectURL={setSelectedSourceURL} /><SourceProbePanel /><DownloadPlanner suggestedURL={selectedSourceURL} onCreated={(plan) => { setPlans((current) => [plan, ...current.filter((item) => item.id !== plan.id)]); setPlansState('ready') }} />{error && <p className="inline-error" role="alert">{error}</p>}{plansState !== 'ready' || !plans.length ? <div className="empty-panel"><Download size={32} /><h2>{plansState === 'loading' ? '正在读取下载任务' : plansState === 'error' ? '暂时无法读取下载任务' : '还没有下载计划'}</h2><p>已有计划会出现在这里，供你查看来源与审批。</p></div> : <>{plans.map((plan) => <article className="download-card" key={plan.id}><header><h2><Download size={19} />{plan.sources.length} 个文件</h2><span className="status-tag">{plan.status}</span></header><p className="download-path">保存到 {plan.targetDirectory}</p><p>预计占用 {formatBytes(plan.estimatedBytes)}</p>{plan.errorCode === 'TASK_INTERRUPTED' && <p className="readonly-error" role="alert">服务重启中断，请重新生成下载计划并审批。</p>}{plan.status === '运行中' && <div className="download-progress"><div><span>{plan.currentSource || '正在准备文件'}<strong>{formatBytes(plan.downloadedBytes ?? 0)} / {formatBytes(plan.estimatedBytes)}</strong></span></div><div className="usage-track"><span style={{ width: `${Math.max(0, Math.min(100, plan.estimatedBytes ? (plan.downloadedBytes ?? 0) / plan.estimatedBytes * 100 : 0))}%` }} /></div></div>}<ul>{plan.sources.map((source, index) => <li key={`${source.url}-${index}`}><strong>{source.title || '未命名来源'}</strong><span className="source-url">{source.url}</span><small>许可：{source.license || '未提供'} · {formatBytes(source.sizeBytes)}</small></li>)}</ul><div className="approval-actions">{plan.status === '待确认' && <><button disabled={approving !== null} onClick={() => void approve(plan, 'deny')}>拒绝</button><button className="primary-button" disabled={approving !== null} onClick={() => void approve(plan, 'approve')}>{approving === plan.id ? '处理中…' : '确认下载'}</button></>}{plan.status === '运行中' && <button className="approval-actions-cancel" disabled={approving !== null} onClick={() => void approve(plan, 'cancel')}>{approving === plan.id ? '处理中…' : '取消下载'}</button>}</div></article>)}{plansHasMore && <button className="text-button history-more" onClick={() => void loadMorePlans()} disabled={plansLoadingMore}>{plansLoadingMore ? '正在读取…' : '加载更早下载任务'}</button>}</>}</section>}
      </main>
