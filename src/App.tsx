    ...
    <div className={`workspace ${collapsed ? 'is-collapsed' : ''}`} style={{ '--accent': accent } as CSSProperties}>
      ...
      <main id="workspace-main" className="workspace-main">
        ...
        {page === 'home' && <section className={`home-content ${activeTask ? 'has-task' : ''}`}>
          {activeTask ? <div className="task-conversation"><button className="text-button" onClick={newTask}>← 返回工作台</button><h1>{activeTask.prompt}</h1><div className="task-answer"><span className="answer-avatar"><Sparkles size={20} /></span><div><strong>NasMate <span className="status-tag">{activeTask.status}</span></strong><p>{activeTask.summary}</p></div>{(activeTask.status === '运行中' || activeTask.status === '规划中') && <button className="task-cancel" onClick={() => void cancelTask(activeTask)}>取消任务</button>}{activeTask.status === '失败' && <button className="secondary-button task-retry" onClick={() => handleSuggestion(activeTask.prompt)}><RotateCcw size={14} />重新发起任务</button>}</div><details className="event-details"><summary><History size={15} />执行轨迹 <span>{events.length} 条记录</span></summary>{eventsState === 'error' ? <p>暂时无法读取轨迹。</p> : eventsState === 'loading' ? <p>正在读取…</p> : events.map((entry) => <div className="event-entry" key={entry.id}><strong>{entry.type}</strong><pre>{JSON.stringify(entry.data, null, 2)}</pre></div>)}</details></div> : <div className="welcome-heading"><span className="welcome-eyebrow"><span />YOUR EVERYDAY NAS COMPANION</span><h1>让琐事简单，<br />让生活<span className="accent-word">多一点空间<span className="heading-spark">✦</span></span>。</h1><p>你好，我是 NasMate。今天，有什么可以一起完成？</p></div>}
