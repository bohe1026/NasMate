...
type DownloadPlan = { id: string; targetDirectory: string; sources: { title: string; url: string; license: string; sizeBytes: number }[]; estimatedBytes: number; status: string; downloadedBytes?: number; currentSource?: string }
...
type SourceProbe = { title: string; url: string; accessible: boolean; statusCode?: number; contentType?: string; sizeBytes?: number; error?: string }
...
function App() {
  ...
  async function probeSource() {
    ...
    if (!url || probingSource) return
    setProbingSource(true); setSourceProbe(null); setError('')
    try {
      const data = await apiFetch('/api/network/sources/probe', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ sources: [{ url }] }) }).then(readJSON<{ items: SourceProbe[] }>)
      setSourceProbe(data.items?.[0] ?? null)
    } catch { setError('来源探测失败，请检查 URL 或稍后重试。') } finally { setProbingSource(false) }
  }
  async function cancelTask(task: Task) {
    ...
    try {
      ...
      setEvents([]); setEventsState('loading')
    ...
    <div className={`workspace ${collapsed ? 'is-collapsed' : ''}`} style={{ '--accent': accent } as CSSProperties}>
      ...
      <main id="workspace-main" className="workspace-main">
        ...
        {page === 'home' && <section className={`home-content ${activeTask ? 'has-task' : ''}`}>
          {activeTask ? <div className="task-conversation"><button className="text-button" onClick={newTask}>← 返回工作台</button><h1>{activeTask.prompt}</h1><div className="task-answer"><span className="answer-avatar"><Sparkles size={20} /></span><div><strong>NasMate <span className="status-tag">{activeTask.status}</span></strong><p>{activeTask.summary}</p></div>{(activeTask.status === '运行中' || activeTask.status === '规划中') && <button className="task-cancel" onClick={() => void cancelTask(activeTask)}>取消任务</button>}</div><details className="event-details"><summary><History size={15} />执行轨迹 <span>{events.length} 条记录</span></summary>{eventsState === 'error' ? <p>暂时无法读取轨迹。</p> : eventsState === 'loading' ? <p>正在读取…</p> : events.map((entry) => <div className="event-entry" key={entry.id}><strong>{entry.type}</strong><pre>{JSON.stringify(entry.data, null, 2)}</pre></div>)}</details></div> : <div className="welcome-heading"><span className="welcome-eyebrow"><span />YOUR EVERYDAY NAS COMPANION</span><h1>让琐事简单，<br />让生活<span className="accent-word">多一点空间<span className="heading-spark">✦</span></span>。</h1><p>你好，我是 NasMate。今天，有什么可以一起完成？</p></div>}
        ...
        {page === 'downloads' && <section className="utility-content"><div className="utility-heading"><span className="welcome-eyebrow">EVERY FILE HAS A HOME</span><h1>下载，心中有数。</h1><p>查看任务状态，并确认每一次文件写入。</p></div><div className="source-probe"><h2>来源安全检查</h2><p>先探测公开 URL，不会下载或写入 NAS。</p><div className="source-probe-row"><input value={sourceURL} onChange={(event) => setSourceURL(event.target.value)} placeholder="https://example.com/file.zip" aria-label="待探测的来源 URL" /><button className="secondary-button" onClick={() => void probeSource()} disabled={!sourceURL.trim() || probingSource}>{probingSource ? "探测中…" : "开始探测"}</button></div>{sourceProbe && <div className={`probe-result ${sourceProbe.accessible ? "is-ok" : "is-error"}`}><strong>{sourceProbe.accessible ? "来源可访问" : "来源不可访问"}</strong><span>{sourceProbe.statusCode ? `HTTP ${sourceProbe.statusCode}` : sourceProbe.error || "未返回状态"}</span>{sourceProbe.contentType && <span>{sourceProbe.contentType}</span>}</div>}</div>{error && <p className="inline-error" role="alert">{error}</p>}{plansState !== 'ready' || !plans.length ? <div className="empty-panel"><Download size={32} /><h2>{plansState === 'loading' ? '正在读取下载任务' : plansState === 'error' ? '暂时无法读取下载任务' : '还没有下载计划'}</h2><p>已有计划会出现在这里，供你查看来源与审批。</p></div> : plans.map((plan) => <article className="download-card" key={plan.id}><header><h2><Download size={19} />{plan.sources.length} 个文件</h2><span className="status-tag">{plan.status}</span></header><p className="download-path">保存到 {plan.targetDirectory}</p><p>预计占用 {formatBytes(plan.estimatedBytes)}</p>{plan.status === '运行中' && <div className="download-progress"><div><span>{plan.currentSource || '正在准备文件'}</span><strong>{formatBytes(plan.downloadedBytes ?? 0)} / {formatBytes(plan.estimatedBytes)}</strong></div><div className="usage-track"><span style={{ width: `${Math.max(0, Math.min(100, plan.estimatedBytes ? (plan.downloadedBytes ?? 0) / plan.estimatedBytes * 100 : 0))}%` }} /></div></div>}<ul>{plan.sources.map((source, index) => <li key={`${source.url}-${index}`}><strong>{source.title || '未命名来源'}</strong><span className="source-url">{source.url}</span><small>许可：{source.license || '未提供'} · {formatBytes(source.sizeBytes)}</small></li>)}</ul><div className="approval-actions">{plan.status === '待确认' && <><button disabled={approving !== null} onClick={() => void approve(plan, 'deny')}>拒绝</button><button className="primary-button" disabled={approving !== null} onClick={() => void approve(plan, 'approve')}>{approving === plan.id ? '处理中…' : '确认下载'}</button></>}{plan.status === '运行中' && <button className="approval-actions-cancel" disabled={approving !== null} onClick={() => void approve(plan, 'cancel')}>{approving === plan.id ? '处理中…' : '取消下载'}</button>}</div></article>)}</section>}
      </main>
